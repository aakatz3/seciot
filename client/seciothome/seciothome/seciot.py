...
class SecIOT():
	def __init__(self, server_host, auth, home_or_mobile, guid = None, statefile="state.db"):
	...
	@staticmethod
	def new_guid(url):
		req = urllib2.Request('http://%s/guid/new' % (url))
...
#	foobar = SecIOT("osrsrv.aakportfolio.com","sfasdasdfa","enc_key" , IOT_HOME_NODE);
	foobar = SecIOT("osrsrv.aakportfolio.com","enc_key" , int(sys.argv[2]), guid="sfasdasdfa")
